...
1. 获取用户给出的卡牌名称或任务范围。
2. 调用 `card-data-finder` 检索本地数据库卡牌数据、中文描述与本地图片 URL。
3. 如果有多个候选，先区分：
   - 哪些是不同逻辑卡
   - 哪些只是同逻辑多卡图 variant
4. 根据任务类型选择路径：
...
- 图片来源默认使用 `card-data-finder` 返回的本地 `image_url`（`http://localhost:3000/api/v1/cards/<id>/image`）。
- 不再使用 GitHub 图床作为运行时卡图来源。
...
1. 用 `card-data-finder` 从本地 `3000` API 取卡牌对象。
2. 确认归属 workspace：
...
1. 用 `card-data-finder` 查同逻辑 variant 列表。
